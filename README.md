...
<p align="center">
  <img src="assets/icon-dotmd-arrow.png" width="128" alt="dotmd Icon">
</p>

## 概要
dotmd は、アウトライン・エディター・プレビューの3画面同期表示に対応した Markdown エディターです。
- Monaco Editor による快適な編集環境（VSCode同等のエディター）
...
### 📝 エディター部
- **Monaco Editor**: VSCode同等の高機能エディター
...
初回起動時に自動的に作成されます。設定を変更する場合は、アプリを終了してから編集してください。

**保存場所**:
- **Windows ポータブル版**:   
    - 設定ファイル(config.json)保存先（`C:\Users\{user}\AppData\Roaming\dotmd\`）  
- **Windows インストーラ版**: 実行ファイルと同じフォルダ
...
### Q: .md ファイルをダブルクリックで開くには？
A: Windows の場合、.md ファイルを右クリック → [プログラムから開く] → [別のプログラムを選択] → dotmd を選択し、[常にこのアプリを使って .md ファイルを開く] にチェックを入れてください。

### Q: 文字化けする場合は？
A: [ファイル] → [文字コード] で適切な文字コードを選択してください。日本語の場合は Shift_JIS または UTF-8(BOM無し) を試してください。

### Q: 外部でファイルが変更された場合は？
A: dotmd は外部でのファイル更新を自動検知します。変更が検出されると、再読み込みするかどうかのダイアログが表示されます。

### Q: 複数のファイルを同時に開けますか？
A: はい、dotmd を複数起動することで、複数のファイルを同時に開くことができます。

### Q: Mac の Intel 版は使えますか？
A: Intel Mac には対応していません。Apple Silicon (M1/M2/M3/M4) 専用です。

### Q: オフラインで使用できますか？
A: はい、基本機能（編集、プレビュー、保存）はオフラインで使用できます。DeepL 翻訳、Tavily 検索、AI チャット機能はインターネット接続が必要です。
...
- **不正な外部送信は一切行いません**
  - アプリケーションが外部に送信するデータは以下のオプション機能のみです（設定した場合のみ）：
    - **DeepL API**: 翻訳テキスト（DeepL 機能を有効化した場合のみ）
    - **Tavily API**: 検索クエリ（Tavily 機能を有効化した場合のみ）
    - **AI プロバイダー API**: チャットメッセージ（AI チャット機能を有効化した場合のみ）
